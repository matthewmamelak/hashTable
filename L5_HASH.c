//LAB5_HASH.C - Lab 05 - Cramarossa, Alexandra and Mamelak, Matthew
...
#include <time.h>
#include <math.h>
#include "C://Users//acram//AppData//Local//Temp//DLL.h"
...
float loadFactor(HashTable *ht) {
//returns a float value of the load factor (population in hash table / size)
    return (float) ht->population / ht->size;
}
...
#define	TABSIZ	42				// define size of table
//42 was chosen as it had both a better ACL and LF than 41
//Though 43 had a better LF, 42 did have a better ACL than 43
//Since the lab deliverable specifies ACL, 42 was chosen

void ACLprint(HashTable *ht){
//sums all chains non-zero length in the hash table and divides by the # of non-zero
//length chains

    //assign variables to the size of the hashtable and sumChain to 0
    int numChains_nonZero = ht->size;
    int sumChain = 0;

    for (int i = 0; i < (ht->size); i++)   {

        if (ht->a[i]->size == 0){   //Checks to see if size of linked list is 0
            numChains_nonZero--;    //If zero, remove one from chain
        }

        else {    //Otherwise add size of the linked list to the chainSum
            sumChain += ht->a[i]->size;
        }
    }

    //print out message showing the ACL
    printf("\nThe Average Chain Length is: %.2f\n", (float)sumChain/numChains_nonZero);
}
...
// second (used in the lab) reads words from a file.
    ...
    char	*p;
    char	buff [132];
    fp = fopen ("C://Users//acram//AppData//Local//Temp//WORDLIST.TXT", "r");
    while (!feof (fp))	{
        fgets (buff, 128, fp);
        p = buff;
        while (*p)	 {
